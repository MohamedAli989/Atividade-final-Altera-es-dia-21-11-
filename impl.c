...
float inicio(float Precogasolina) {
  printf(BLU "=-= " BYEL "Bem vindo ao Posto da Esquina!" BLU
             " =-= \n\n=-= " BYEL "Projeto de Mohamed Ali "
             "Awale " BLU "=-=\n\n=-=" BYEL
             " Este programa irá organizar o funcionamento de um "
             "Posto de Gasolina com uma bomba de combustível " BLU "=-=" WHT);

  printf("\n\n" UGRN
         "Qual é o valor da gasolina hoje?\n(Por favor insira ponto (.) "
         "no lugar da vírgula (,))." WHT "\nDigite: ");
  scanf("%f", &Precogasolina);
  while (Precogasolina <= 0) {
    printf(RED
           "\n\n" URED "Valor inválido (número negativo ou Zero)." UGRN
           "\nQual é o valor da "
...
void abastecerTanque(float *Tgasolina) {
  if (*Tgasolina != 200) {
    printf(GRN "Tanque de Gasolina Abastecido!\n\n" WHT);
    *Tgasolina = 200;
  } else {
    printf(UYEL "O Tanque já está cheio!\n\n" WHT);
  }
}
////Tabelas
...
void TabelaComCarro(float *Somavendida, float *SomaTotal, int *CAbastecido,
                    float *Tgasolina, int carroresto) {
  if (*CAbastecido == 1) {
...
void ArquivoDados(float *Somavendida, float *SomaTotal, int *CAbastecido,
                  float *Tgasolina, FILE *A, int carroresto) {
  if (*CAbastecido == 0) {
